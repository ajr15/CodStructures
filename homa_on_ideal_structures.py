...
def make_planar_structure(stype: str):
    mol = utils.get_molecule(f"displaced_structures/{stype}/Propellering.mol2")
    ...
    conv.WriteFile(mol, f"displaced_structures/{stype}/plannar.xyz")

def get_mode_mol(mode: str, stype: str):
    return utils.get_molecule(f"displaced_structures/{stype}/{mode}.mol2")

def get_planar_mol(stype: str):
    return utils.get_molecule(f"displaced_structures/{stype}/plannar.xyz")
...
def calc_homa_dict(mol: ob.OBMol, stype: str):
    ajr = {}
    definition_mol = utils.get_molecule(os.path.join("data", "definitions", stype + ".mol"))
...
def porphystruct_out_of_plane(mol: ob.OBMol):
    ...
    os.system("./porphystruct/PorphyStruct.CLI analyze -x test/tmp.mol2 > /dev/null")
