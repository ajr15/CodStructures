...
class SubstituentPropertyFeaturizer (Featurizer):
    ...
    def featurize(self, session, structure_ids, property_names, property_units) -> np.array:
        res = []
        for sid in structure_ids:
            vec = {}
            for pname, punits in zip(property_names, property_units):
                vec.update(self.structure_property(session, sid, pname, punits))
            res.append(vec)
        df = pd.DataFrame(res)
    ...
    @staticmethod
    def structure_property(session, sid: int, prop: str, units: str):
        ...
        if len(rows) == 0:
            ...
            ajr = {"{} {}{}".format(prop, v[0], v[1]): v[2] for v in rows}
            return ajr
